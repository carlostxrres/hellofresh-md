...
function main() {
  ...
  const nutrition100gActive = document.querySelector('[aria-pressed="true"] [data-translation-id="recipe-detail.recipe-detail.per-100g"]')
  ...
  const nutrition = scrapeNutrition()
  const description = scrapeDescription()
  const url = location.href
  const pdfUrl = document.querySelector('[data-test-id="recipe-pdf"]').href
  ...
  const utensils = scrapeUtensils()
  const utensilList = arrayToMarkdownList(utensils)

  const utensilListIndented = "\n" + arrayToMarkdownList(utensils, 2)

  // to do: pass to markdown using a library (has to keep text formatting: paragraphs, bold...)
  const instructionsHtml = scrapeInstructionsHtml()
...
6. En una ensaladera, mezcla el **vinagre balsámico**, un chorrito de **aceite, sal** y **pimienta**. Luego, añade la **rúcula** y remueve para aderezar. Sirve el **puré de patata** y la **ensalada de rúcula** por separado en platos. Luego, coloca la **merluza** en el plato y cubre con la **salsa de naranja y miel**.`
  ...
  const allergens = scrapeAllergens()
